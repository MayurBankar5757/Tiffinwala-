...
function UpdateUserForm() {
  ...
  // If user or token is not available, redirect to login
  useEffect(() => {
    ...
    // Fetch the existing user data using the JWT token in the header
    fetch(`http://localhost:8103/api/users/${uid}`, {
      headers: {
      ...
      .then((data) => {
        setFormData({
          ...
          lname: data.lname,
          email: data.email,
          roleId: data.role.roleId, // Set roleId
      ...
      fetch(`http://localhost:8103/api/users/${uid}`, {
        ...
        .then((response) => {
          if (response.ok) {
            ...
            navigate("/vendor_home"); // Redirect after update
          } else {
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  ...
                  <input
                    type="text"
                    className="form-control form-control-sm"
                    value={formData.roleId === 2 ? "Vendor" : "Customer"}
                    readOnly
                    style={{
                      backgroundColor: "#f7f7f7",
                      color: "#6c757d",
